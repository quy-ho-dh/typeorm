...
async function createYamlFiles() {
    if (!(0, fs_1.existsSync)(path_1.default.join(__dirname, "configs/yaml"))) {
        await fs_1.promises.mkdir(path_1.default.join(__dirname, "configs/yaml"));
    }
    await fs_1.promises.writeFile(path_1.default.join(__dirname, "configs/yaml/test-yaml.yaml"), '- type: "sqlite"\n  name: "file"\n  database: "test-yaml"');
}
describe("ConnectionOptionsReader", () => {
    ...
    it("properly loads config from yaml", async () => {
        ...
        const connectionOptionsReader = new ConnectionOptionsReader_1.ConnectionOptionsReader({
            root: path_1.default.join(__dirname, "configs/yaml"),
            configName: "test-yaml",
        });
        const fileOptions = await connectionOptionsReader.get("file");
        (0, chai_1.expect)(fileOptions.database).to.have.string("/test-yaml");
    });
});
